...
import { TelemetryClient, telegrafMiddleware } from '@titorelli/telemetry-client'
...
export class Bot {
  ...
  constructor({
    ...
    this.titorelli = new TitorelliClient({
      ...
      modelId: 'generic',
      scope: ['predict', 'train', 'exact_match/train', 'totems/train', 'cas/predict', 'cas/train']
    })
  }
  ...
  private installMessageHandler() {
    this.bot.on('message', async (ctx, next) => {
      const text = deunionize(ctx.message).text || deunionize(ctx.message).caption
      const { message_id: messageId, from: { id: fromId } } = ctx.message
      ...
      const { value: label, reason } = await this.titorelli.predict({ text, tgUserId: fromId })
